...
// Version asset URLs so a returning phone cannot mix new HTML with cached code.
const html=(await readFile('dist/index.html','utf8')).replace('href="./styles.css"',`href="./styles.css?v=${hash}"`).replace('src="./app.js"',`src="./app.js?v=${hash}"`);
await writeFile('dist/index.html',html);
const sw=`const CACHE='cockpit-${hash}';const ASSETS=['./','./index.html','./app.js?v=${hash}','./styles.css?v=${hash}','./icon.svg','./manifest.webmanifest'];self.addEventListener('install',e=>e.waitUntil(caches.open(CACHE).then(c=>c.addAll(ASSETS)).then(()=>self.skipWaiting())));self.addEventListener('activate',e=>e.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k.startsWith('cockpit-')&&k!==CACHE).map(k=>caches.delete(k)))).then(()=>self.clients.claim())));self.addEventListener('fetch',e=>{if(e.request.method!=='GET'||new URL(e.request.url).origin!==self.location.origin)return;e.respondWith(fetch(e.request).then(r=>{if(r.ok){const copy=r.clone();e.waitUntil(caches.open(CACHE).then(c=>c.put(e.request,copy)));}return r;}).catch(()=>caches.match(e.request).then(r=>r||(e.request.mode==='navigate'?caches.match('./index.html'):Response.error()))));});`;
