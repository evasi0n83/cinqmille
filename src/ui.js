import {scoreOf,livesOf,currentStep,fallbackOf,ranking} from './game.js';
...
function checkpointTrack(player) {
  if(!player.steps.length)return '<p class="checkpoint-empty">Aucun palier · entrée à 400</p>';
  const current=currentStep(player),fallback=fallbackOf(player);
  return `<ol class="checkpoint-track" aria-label="Paliers de ${escape(player.name)}, du plus ancien au plus récent">${player.steps.map(step=>{
    const barred=step.barred||step.lives===0;
    const state=barred?'barred':step.id===current?.id?'current':step.id===fallback?.id?'fallback':'valid';
    const label=barred?'Barré':state==='current'?'Actuel':state==='fallback'?'Repli':'';
    return `<li class="checkpoint ${state}"><span class="checkpoint-score">${barred?`<s>${fmt(step.score)}</s>`:fmt(step.score)}<span class="visually-hidden"> points</span></span><span class="checkpoint-detail">${barred?'':hearts(step.lives)}${label?`<span class="checkpoint-state">${label}</span>`:''}</span>${barred?`<span class="visually-hidden"> : ${escape(step.reason||'Plus de vies')}</span>`:''}</li>`;
  }).join('')}</ol>${!fallback?'<span class="checkpoint-zero">Repli à 0</span>':''}`;
}
...
export function renderGame(game,{spectator,busy,draft,liveStatus,message}) {
  ...
  return `<div class="game-grid">
    ...
    <section class="leaderboard leaderboard--paliers" data-density="rows" aria-label="Classement et paliers">
      <div class="section-heading"><h2>Classement</h2><span>${winner?'PARTIE TERMINÉE':'OBJECTIF 5 000'}</span></div>
      <div class="player-list">${ranking(game).map(({player:p,rank})=>`<article class="player-card ${p.id===player.id&&!winner?'active':''} ${rank===1&&scoreOf(p)>0?'leader':''}" aria-label="${escape(p.name)}"><button type="button" class="player-row ${p.id===player.id&&!winner?'active':''} ${rank===1&&scoreOf(p)>0?'leader':''}" data-player="${p.id}" aria-haspopup="dialog" aria-label="${escape(p.name)}, ${fmt(scoreOf(p))} points, ${livesOf(p)} vies, voir le détail des paliers">
      ...
      </button><div class="player-checkpoints">${checkpointTrack(p)}</div></article>`).join('')}</div>
    </section>
  </div>`;
}
